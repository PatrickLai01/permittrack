...
/**
 * Returns a sorted list of distinct county names from the permits table.
 * Calls the get_distinct_counties() Postgres function via RPC.
 */
export async function getDistinctCounties() {
  const { data, error } = await supabase.rpc('get_distinct_counties')
...
/**
 * Returns a sorted list of distinct municipalities with permit counts.
 * Queries the permits table directly for municipality_name values.
 */
export async function getDistinctMunicipalities() {
  ...
  // Group by municipality_name and count
  const counts = {}
  ;(data ?? []).forEach((row) => {
...
export async function searchPermit(rawQuery, cityId = null) {
  ...
  const SELECT =
    'permit_number, address, status, description, applied_date, issued_date, finalized_date, municipality_id'

  // Build base query
  let builder = supabase.from('permits').select(SELECT)

  // Narrow to city when one is chosen
  if (cityId) builder = builder.eq('municipality_id', cityId)
